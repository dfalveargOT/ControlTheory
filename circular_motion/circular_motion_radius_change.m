%%%%% Circular motion with control for radius change
...
T = 20;
tspan = [0 T];
[t,x_sol] = ode45(@(t,x) V_circular_with_control(t,x), tspan, x0);
...
function dx = V_circular_with_control(t, x)
    A = [0 -1;
         1 0];
    B = [0; 1];
    K = -[-0.4142    1.3522];
    
    dx = A*x + B*K*(x - [2*cos(t); 2*sin(t)]) ;
